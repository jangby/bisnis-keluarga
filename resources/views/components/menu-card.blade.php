@props(['href', 'color', 'icon', 'title', 'subtitle'])

@php
    // Ikon SVG Sederhana (Heroicons)
    $icons = [
...
<a href="{{ $href }}" wire:navigate class="group relative bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-{{ $color }}-200 transition-all duration-300 flex flex-col items-center text-center overflow-hidden h-full">
    {{-- Efek Hover Background --}}
    <div class="absolute inset-0 bg-{{ $color }}-50 opacity-0 group-hover:opacity-100 transition duration-300"></div>
    
    {{-- Icon --}}
    <div class="relative z-10 w-14 h-14 rounded-2xl bg-{{ $color }}-100 text-{{ $color }}-600 flex items-center justify-center mb-4 group-hover:scale-110 transition duration-300 shadow-inner group-hover:shadow-md">
    ...
    {{-- Teks --}}
    <h3 class="relative z-10 text-base font-bold text-gray-800 group-hover:text-{{ $color }}-700 transition">{{ $title }}</h3>
    <p class="relative z-10 text-xs text-gray-400 group-hover:text-{{ $color }}-600/80 transition mt-1">{{ $subtitle }}</p>
</a>